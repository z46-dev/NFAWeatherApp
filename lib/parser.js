...
export function parse(originalBuffer) {
    ...
    const format = new Format(buffer, [
        ...
        ["u16LE", "Storm Rain", x => (x / 100) + " in"],
        ["u16LE", "Storm Start Date", x => new Date(2000 + (x >> 9), (x >> 5) & 0xF, x & 0x1F).toLocaleDateString()],
        ["u16LE", "Day Rain", x => (x / 100) + " in"],
        ["u16LE", "Month Rain", x => (x / 100) + " in"],
        ["u16LE", "Year Rain", x => (x / 100) + " in"],
        ["u16LE", "Day ET", x => (x / 100) + " in"],
        ["u16LE", "Month ET", x => (x / 100) + " in"],
        ["u16LE", "Year ET", x => (x / 100) + " in"],
        ...
        ["u8", "Transmitter Battery Status"],
        ["u16BE", "Console Battery Voltage", x => ((x * 300) / 512) / 100],
        ["u8", "Forecast Icons"],
        ["u8", "Forecast Rule Number"],
        ["u16BE", "Time of Sunrise", x => new Date(0, 0, 0, x >> 8, x & 0xFF).toLocaleTimeString()],
        ["u16BE", "Time of Sunset", x => new Date(0, 0, 0, x >> 8, x & 0xFF).toLocaleTimeString()]
    ]);
